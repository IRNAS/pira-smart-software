...
class JoinAcceptPayload:
    ...
    def get_devaddr(self):
        return list(map(int, bytearray(reversed(self.devaddr))))
    ...
    def compute_mic(self, key, direction, mhdr):
        ...
        cmac = AES_CMAC()
        computed_mic = cmac.encode(bytes(bytearray(key)), bytes(bytearray(mic)))[:4]
        return list(map(int, bytearray(computed_mic)))
    ...
    def decrypt_payload(self, key, direction, mic):
        ...
        self.payload = cipher.encrypt(bytes(bytearray(a)))[:-4]
        ...
        return list(map(int, bytearray(self.payload)))
    ...
    def encrypt_payload(self, key, direction, mhdr):
        ...
        cipher = AES.new(bytes(bytearray(key)))
        return list(map(int, bytearray(cipher.decrypt(bytes(bytearray(a))))))
    ...
    def derive_nwskey(self, key, devnonce):
        ...
        cipher = AES.new(bytes(bytearray(key)))
        return list(map(int, bytearray(cipher.encrypt(bytes(bytearray(a))))))
